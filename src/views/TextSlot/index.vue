<template>
  <v-container>
    <v-text-field label="Name" outlined v-model="baseInfo.name" maxlength="50" @input="validateName">
...
export default class TextSlotIndex extends Vue {
  ...
  validateName(): void {
    ...
    if (this.baseInfo.name.trim().length > 0) {
      this.hasBeenUsed = this.persons.some((item) => item.name.toUpperCase() === this.baseInfo.name.trim().toUpperCase())
    }
  }
}
</script>